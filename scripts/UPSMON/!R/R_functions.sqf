...
R_ThrowSmoke = 
	{
		private ["_unit","_shell",
					"_moves","_unitPos","_pos","_flare","_direction","_z"];
		...
		*/
		...
		_pos = position _unit;
		_pos = [(_pos select 0),(_pos select 1),(_pos select 2)+_z];
			...
			if ( (_dist) <= _atdist ) then {			
				_vehicles = [_npc] call R_FN_vehiclesUsedByGroup; // vehicles use by the group			
				{
					...
					// All units disembark if the vehicle is a Car
						if ((_x iskindof "CAR" && (_x emptyPositions "GUNNER") == 0) || (_x iskindof "TRUCK" && (_x emptyPositions "GUNNER") == 0)) then
						{
	...
	R_FN_GOTHIT =
	{
		...
		if ((count R_GOTHIT_ARRAY) > 0 || (count R_GOTKILL_ARRAY) > 0) then
		{
			true
		}
		else
	...
	R_SN_EHHIT = 
	{
		...
		if ((side _unit != side _shooter) && !(_grp in R_GOTHIT_ARRAY)) then
		{
			R_GOTHIT_ARRAY = R_GOTHIT_ARRAY + [_grp]; 
	...
	R_SN_EHKILLED = 
	{
		...
		if ((side _unit != side _shooter) && !(_grp in R_GOTKILL_ARRAY)) then
		{
			R_GOTKILL_ARRAY = R_GOTKILL_ARRAY + [_grp]; 
			// if (KRON_UPS_Debug > 0) then {player globalchat format["UNIT: %1, SHOOTER :%2 %3",_unit,_shooter,side _shooter]};
		};
	};
